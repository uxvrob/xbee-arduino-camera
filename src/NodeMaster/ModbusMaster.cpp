...
#include "ModbusMaster.h"
  ...
  USARTSerial ModbusMaster::MBSerial = Serial1;     ///< Pointer to Serial1 class object
...
//uint8_t ModbusMaster::MBTxEnablePin = D7;      ///< GPIO pin used for toggling RS485 Driver IC's TX Enable pin, default is D7
//uint8_t ModbusMaster::MBUseEnablePin = 0;      ///< Should a TX_ENABLE pin be used? 0 = No, 1 = Yes
//uint8_t ModbusMaster::MBDebugSerialPrint = 1;    ///< Do you want the TX and RX fraimes printed out on Serial for debugging? 0 = No, 1 = Yes
...
#endif
{
  ...
  //ModbusMaster::_u8SerialPort = 1;
  ModbusMaster::_u8MBSlave = u8MBSlave;
...
void ModbusMaster::enableDebug() {
	ModbusMaster::MBDebugSerialPrint = 1;
}
...
void ModbusMaster::disableDebug() {
	ModbusMaster::MBDebugSerialPrint = 0;
}
...
@ingroup setup
*/
void ModbusMaster::enableTXpin(int pin) {
	pinMode(pin, OUTPUT);
	ModbusMaster::MBTxEnablePin = pin;
	ModbusMaster::MBUseEnablePin = 1;
...
uint8_t ModbusMaster::ModbusMasterTransaction(uint8_t u8MBFunction) {
	...
	// Flush receive buffer before transmitting
	...
	// ---Transmit Command--- 
	if (ModbusMaster::MBUseEnablePin == 1) {  //Switch RS485 driver to transmitting mode.
		digitalWrite(ModbusMaster::MBTxEnablePin, HIGH);
	}
	...
	if(ModbusMaster::MBDebugSerialPrint == 1) { // Print received frame for Debugging purposes out on Serial
		Serial.println(F(" "));
		Serial.print(F("RX: "));
	}

	if (ModbusMaster::MBUseEnablePin == 1) {  //Switch RS485 driver back to receiving mode.
