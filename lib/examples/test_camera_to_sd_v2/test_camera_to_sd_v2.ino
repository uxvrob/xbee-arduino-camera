...
// Arduino Ethernet shield: pin 4 <<<<**RMH** This is what we have >>>>
...
SoftwareSerial cameraconnection = SoftwareSerial(2, 3); // <<<<**RMH** This is the original line >>>>
//SoftwareSerial cameraconnection = SoftwareSerial(1, 0); // <<<<**RMH** Setting this to default TX/RX pins >>>>
// On Mega: camera TX connected to pin 69 (A15), camera RX to pin 3:
//SoftwareSerial cameraconnection = SoftwareSerial(69, 3);
#else
NewSoftSerial cameraconnection = NewSoftSerial(2, 3); //<<<<**RMH** Can be ignored >>>>
#endif
...
// <<<<**RMH** Add a signal to light up an LED while taking the picture >>>>
...
void setup() {
  ...
  // <<<< **RMH** Note that if you want to change this, you need to power cycle the camera. Otherwise it won't change the setting >>>>
...
void loop() {
  ...
  // Take a picture if the button is pushed
  if (buttonState == HIGH) {
    // turn LED on:
    ...
    // <<<<**RMH** Blink the LED one more time to indicate write is complete if not connected to serial monitor >>>>
    digitalWrite(LED_OUTPUT_PIN,HIGH);
    delay(1000); 
    digitalWrite(LED_OUTPUT_PIN,LOW);
  }
